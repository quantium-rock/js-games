...
const getBalance = async () => {
  ...
  var phantom = provider();
  const connection = new solanaWeb3.Connection(solanaWeb3.clusterApiUrl('devnet'), 'confirmed');
  let balance = await connection.getBalance(phantom.publicKey) / solanaWeb3.LAMPORTS_PER_SOL;
  console.log(balance)
  return balance
}
  ...
  if (betSum == 0) {
    ...
    const resultsDisplay = () => {
      ...
      if (winAmountOnScreen > 0) {
        $(".win-amount").html(`◎${winAmountOnScreen}`);
      } else {
        $(".win-amount").html("");
      }
      bankSum = cashSum;
  ...
  for (let i = 1; i <= 10; i++) {
    (function (i) {
      setTimeout(function () {
        ...
        $(".cash-total").html(`${Math.round(cashSumBefore)}`);
      }, 50 * i);
    })(i);
  }
